  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/schedule/${scheduleToDelete}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
              ...
              schedules.map((schedule) => {
                ...
                <tr key={schedule.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {court?.name || schedule.court_name || `Quadra ${schedule.court_id}`}
                  </td>
...
function FormSchedule({
  ...
  const form = useForm({
    ...
    defaultValues: {
      court_id: editingSchedule?.court_id.toString() || "",
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      const courtId = parseInt(values.court_id);
      const url = editingSchedule
        ? `http://localhost:8000/schedule/${editingSchedule.id}`
        : `http://localhost:8000/schedule/?court_id=${courtId}`;
